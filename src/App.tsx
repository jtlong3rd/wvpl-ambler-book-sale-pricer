...
const bagSaleItemSpec: ItemSpec<BagSale> = {
  key: "fill-a-bag",
  name: "Fill a Bag",
  offer: (state) => state === "early-afternoon" || state === "late-afternoon",
  renderPricingDescription: (state) =>
    state === "early-afternoon" ? "$8 / bag" : "$5 / bag",
  pricingRule: (count, state) =>
    state === "early-afternoon" ? count * 8 : count * 5,
};
...
function App() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {itemSpecs
        .filter((itemSpec) => itemSpec.offer == null || itemSpec.offer(bagSale))
        .map((itemSpec, i) => (
            ...
            <div>
              <Item
                ...
                onDecrement={() =>
                  setItemCounts((itemCounts) => ({
                    ...itemCounts,
                    [itemSpec.key]: Math.max(itemCounts[itemSpec.key] - 1, 0),
                  }))
                }
                onIncrement={() =>
                  setItemCounts((itemCounts) => ({
                    ...itemCounts,
                    [itemSpec.key]: itemCounts[itemSpec.key] + 1,
                  }))
                }
